...
## Iteration 6 — Refactor for SOLID clarity (no behavior change)

**Date:** 2026-08-29

**Goal:** Clean up the architecture without changing user-visible behavior. The purpose is to reduce single-responsibility violations and validation drift while keeping the existing API contract and runtime setup stable.

**Refactored:**
- Extracted `AliasValidator` to own custom alias validation and reserved-name checks.
- Extracted `LinkExpiryPolicy` to own TTL resolution and expiry evaluation.
- Reduced the responsibilities of `UrlShortenerService` so it focuses on orchestration rather than owning all validation and business rules directly.
- Kept the previous route behavior, error responses, and Compose-based runtime configuration intact.

**No behavior change:**
- The app still accepts the same request payloads, returns the same status codes, and preserves the current alias and expiry semantics.
- This was a structural cleanup, not a feature expansion.

**Why this matters:**
- The service layer was taking on too many concerns at once, which made validation and lifecycle rules harder to reason about.
- Validation logic had begun to drift across the service boundary and could become inconsistently applied over time.
- The refactor isolates each concern so future iterations are easier to extend without unintended side effects.

**Verification:**
- The full Maven test suite was re-run after the refactor to confirm no regression in shortening, alias handling, expiry enforcement, stats, or duplicate detection.

**Changed files:**
- `src/main/java/com/schwab/urlshortener/service/AliasValidator.java`
- `src/main/java/com/schwab/urlshortener/service/LinkExpiryPolicy.java`
- `src/main/java/com/schwab/urlshortener/service/UrlShortenerService.java`

**AI-assistance notes:** This is intentionally a refactor-only iteration. The goal is architectural clarity and maintainability, not new functionality, and the documentation explicitly calls out that fact so it is not mistaken for a scope expansion.
